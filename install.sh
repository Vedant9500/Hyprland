...
makepkg -si
cd
mkdir Downloads Documents Pictures
sudo pacman -Syu --noconfirm noto-fonts noto-fonts-emoji ffmpeg xdotool zsh git vim bluez-utils neofetch mpc mpv ntfs-3g thunar kitty
systemctl enable bluetooth
echo "Installing applications"
sudo pacman -Syu --noconfirm discord telegram-desktop
sudo usermod -a -G input $USER
sudo usermod -aG audio $USER
yay -Syu hyprland swww dolphin google-chrome wine

pacman -Sy git
git clone --depth 1 https://github.com/prasanthrangan/hyprdots ~/HyDE
cd ~/HyDE/Scripts
./install.shpacman -Sy git
